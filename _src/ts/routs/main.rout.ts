...
mainRout.get('/', (req, res) => {
  res.send(
    `<main style="text-align: center; margin: 30vh 0;"><h1>Главная страница приложения Balance-rating</h1>
            ...
            используйте запрос: http://localhost:3000/max-balance-acc
...
// предыдущая реализация метода от 01062023
mainRout.get('/max-balance-acc', (req, res) => {
  const ctr = new MainController();
  ctr.getMaxChangedAccount(req, res);
});

// реализация от 02062023
mainRout.get('/max-balance-new', (req, res) => {
  const ctr = new NewController();
  ctr.getTheMaxChangedAccount(req, res);
});
